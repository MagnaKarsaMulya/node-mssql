...
class Request extends EventEmitter {
  ...
  query (command, callback) {
    ...
    for (const i in this.parameters) {
      query_values.push(this.parameters[i].value);
    }
    ...
    if (typeof callback === 'function') {
      this._query(command, (err, recordsets, output, rowsAffected, columns) => {
        ...
        if (err) {
          this.emit('query', command, query_values, process.hrtime(this.timer), err)
          return callback(err)
        }
        ...
        this.emit('query', command, query_values, process.hrtime(this.timer), null)
