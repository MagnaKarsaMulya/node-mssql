...
class ConnectionPool extends EventEmitter {
  ...
  acquire (requester, callback) {
    ...
    acquirePromise.then(connection => {
      if (this.pool && typeof this.pool.used != 'undefined') {
        this.emit('acquire', this.pool.used.length, this.pool.used.length + this.pool.free.length, this.pool.pendingAcquires.length);
      }
      return connection;
    })
    if (typeof callback === 'function') {
  ...
  release (connection) {
    ...
    if (this.pool) {
      ...
      this.emit('release', this.pool.used.length, this.pool.used.length + this.pool.free.length, this.pool.pendingAcquires.length);
    }
    return this
  }
  ...
  request () {
    let new_request = new shared.driver.Request(this)
    this.emit('connection', new_request)
    return new_request
  }
  ...
  transaction () {
    let new_transaction = new shared.driver.Transaction(this)
    this.emit('transaction', new_transaction)
